...
class MedicoAgendaView(APIView):
    ...
    def get(self,request):
        ...
        lista_agendamentos = Agenda.objects.filter(uuid_medico  =informacoes_token['uuid_user'])
    ...
    def post(self, request):
        ...
        lista_agendamentos = Agenda.objects.filter(uuid_medico=informacoes_token['uuid_user']).filter(Q(status="disponivel") | Q(status="aguardando_confirmacao") | Q(status="confirmada"))
...
class MedicoAgendaDetalhesView(APIView):
    ...
    def get(self, request, uuid):
        try:
            ...
            return Response(data=e.__str__(), status=status.HTTP_403_FORBIDDEN)
        try:
            agenda = Agenda.objects.get(uuid=uuid, uuid_medico=informacoes_token['uuid_user'])
